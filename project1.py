...
dist_transform = ((dist_transform - dist_transform.min()) / (dist_transform.max() - dist_transform.min()) * 255).astype(
    np.uint8)
# cv2.imwrite('3_dilated_eroded_background.jpg', sure_bg)
# cv2.imwrite('4_distance_transformed.jpg', dist_transform)
...
# cv2.imwrite('6_segmented_regions_colored.jpg', img_3C)
...
for cont in contours:
    ...
    if not (not (w > 30) or not (h > 50)) and h < 0.75 * imgH and w < 0.75 * imgW:
        # for debugging purposes (finding out why some bubbles are not detected)
        # cv2.drawContours(new_cont,[cont], -1, (0,255,0), 3)
        # plt.imshow(new_cont, interpolation='bicubic')
        ...
        if is_text:
            cont_counter += 1
            cv2.drawContours(new_cont, [cont], -1, (0, 255, 0), 3)
            # cv2.imwrite('written_contour' + str(cont_counter) + '.jpg', new_cont)
...
for bubble in list_of_bubbles:
   cv2.drawContours(img, [bubble], -1, (255,255,255), thickness = -1)

status = cv2.imwrite('9_output_.jpg', img)
